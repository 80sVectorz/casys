...
from casys.dsl._core import (
    macro_handlers,
)  # Needs to be last to ensure macro handlers are registered  # noqa: F401


def handle_macros_recursive(
    name, target_ir: Ir_CaKernel | Ir_SimStepFunc, sys_ir: Ir_CaSys
):
    ...
    while finder is None or finder.matches:
        ptrn_un_handled_macros = [
            Collect(
                pattern=Filter(
                    lambda n: isinstance(n, casys_ast.Cs_Macro)
                    and id(n) not in handled_nodes
                ),
                key="macro",
            )
        ]
    ...
    def process(self, ir: Ir_CaSys) -> None:
            ...
            Collect(
                pattern=NodePattern(
                    ...
                    func=NodePattern(
                        node_type=ast.Name,
                        id=Filter(
                            predicate=lambda n: n in core_macros._MACROS, key="name"
                        ),
                    ),
                ),
                key="call",
                ...
                match_in_expr(
                    Collect(
                        pattern=NodePattern(
                            ...
                            func=NodePattern(node_type=ast.Name, id=Bind("name")),
                        ),
                        key="call",
                    )
                ),
        ...
        def mark_macro(m: dict[str, Any]) -> list[ast.AST]:
            ...
            new_node = (
                ...
                if not isinstance(m["call"], casys_ast.Cs_Macro)
                else call_node
            )
        ...
        PatternTransformer(
            pattern=ptrn_macro_calls, actions={"call": mark_macro}
        ).visit(self.target_ir.ir_ast)

        # Remove enclosing Expr node
        PatternTransformer(
            pattern=ptrn_expr_calls, actions={"expr": lambda m: [m["call"]]}
        ).visit(self.target_ir.ir_ast)

        trkr.add_snapshot(
            ast_node=self.target_ir.ir_ast,
            tags=(
                f_tag_kernel(self.target_name)
                if isinstance(self.target_ir, Ir_CaKernel)
                else TAG_STEP_FUNC,
    ...
    def process(self, ir: Ir_CaSys) -> None:
        ...
        transformers = (
            BottomUpPatternTransformer(
                ...
                actions={
                    "call": lambda m: core_macros._MACRO_HANDLERS[m["name"]](
                        m["call"], ir
                    )
                },
            ),
            ...
            BottomUpPatternTransformer(
                ...
                actions={"node": lambda m: [m["body"]] if m["test"] else [m["orelse"]]},
            ),
            BottomUpPatternTransformer(
            ...
            BottomUpPatternTransformer(
                ...
                actions={"node": lambda m: [m["node"]] if m["body"] else m["orelse"]},
            ),
        )

        for tf in transformers:
            tf.visit(self.target_ir.ir_ast)
