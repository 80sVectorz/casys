...
from numpy.f2py.auxfuncs import isthreadsafe
...
class InsertBoundsLogic(TranspilerModule):
    """Insert bounds handling logic for positional variable overwrites and subscript indices."""
    ...
    def process(self, ir: Ir_CaSys) -> None:
        ...
        def handle_assign(m: dict[str, Any]) -> list[ast.AST]:
            ...
            if (m_targets:=ptrn_targets_tuple.match(targets_list[0])) :
                ...
                for target, value in zip(targets, values):
                    ...
                    new_values.append(snippet_wrap_logic(value, target_ax))
                ...
                assign.value = snippet_wrap_logic(assign.value,target_ax)
                casys_ast.get_meta(assign).verified_bounds = True
        ...
        def handle_walrus_assign(m: dict[str, Any]) -> list[ast.AST]:
            ...
            assign.value = snippet_wrap_logic(value,target_ax)
            casys_ast.get_meta(assign).verified_bounds = True
            return [assign]
