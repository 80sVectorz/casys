...
from casys.dsl._core.layer_usage_info_helper import UnfinishedLayerUsageInfo

class AnalyzeLayerFieldsUsage(TranspilerModule):
    def process(self, ir: Ir_CaSys) -> None:
        trkr = get_tracker()
        trkr.enter_phase('Analyzing kernel layer and layer fields usage')
        ...
        ptrn_layer_field_ref = [
            Collect(
                pattern=NodePattern(
                    node_type=ast.Subscript,
                    value=NodePattern(casys_ast.Cs_LayerFieldRef, b=Bind('b'), f=Bind('f')),
                    ctx = Bind('ctx'),
                    slice = Bind('slice')
                ),
                key='subscript'
        ...
        for name, kernel in ir.kernels.items():
            ...
            layer_usage_info = UnfinishedLayerUsageInfo(kernel)

            (finder := FindGuaranteedLayerFieldAccesses()).visit(kernel.ir_ast)
            guaranteed_field_writes, guaranteed_field_reads = finder.get_guaranteed_reads_and_writes()

            (finder:=PatternFinder(ptrn_layer_field_ref)).visit(kernel.ir_ast)
            ...
            for m in finder.matches:
                ...
                match m['ctx']:
                    case ast.Load():
                        layer_usage_info.add_read(l,f, is_local=is_local)

                    case ast.Store():
                        is_guaranteed = (l,f) in guaranteed_field_writes
                        layer_usage_info.add_write(l,f, guaranteed=is_guaranteed)

            kernel.metadata.set(MDK_SOA_FIELD_USAGE_INFO, layer_usage_info.finalized())
...
class FindGuaranteedLayerFieldAccesses(ast.NodeVisitor):
    ...
    def visit_Subscript(self, node: ast.Subscript) -> None:
        if isinstance(node.value, casys_ast.Cs_LayerFieldRef):
            match node.ctx:
                case ast.Load():
                    if self.conditional_depth == 0:
                        self.reads.add((node.value.l,node.value.f))
                    else:
                        self.conditional_reads[-1].add((node.value.l,node.value.f))
                case ast.Store():
                    if self.conditional_depth == 0:
                        self.writes.add((node.value.l,node.value.f))
                    else:
                        self.conditional_writes[-1].add((node.value.l,node.value.f))
